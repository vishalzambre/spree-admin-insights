...
describe Spree::BestSellingProductsReport do
  let(:start_date) { (Date.today - 10.days).to_s }
  let(:end_date) { Date.today.to_s }
  let(:search_params) { { search: { start_date: start_date, end_date: end_date, name: 'ruby' } } }
  let(:report) { Spree::BestSellingProductsReport.new(search_params) }
  ...
  describe '#initialize' do
    ...
    it { expect(report.instance_variable_get(:@name)).to eq('%ruby%') }
